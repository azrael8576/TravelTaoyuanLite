...
#### Backend
- [_桃園觀光導覽網 Open API_](https://travel.tycg.gov.tw/open-api/swagger/ui/index#/Event/Event_News): -   Provides tourism data from Taoyuan City, including events, attractions, and news.
...
## Types of modules in 旅桃Lite
![image](https://github.com/azrael8576/TravelTaoyuanLite/blob/main/docs/images/modularization-graph.drawio.png)  
...
採用上述模組化策略，旅桃Lite 應用程序具有以下模組：

| Name | Responsibilities | Key classes and good examples |    
|:----:|:----:|:-----------------:|    
| `app` | 將所有必要元素整合在一起，確保應用程式的正確運作。<br>eg. UI scaffolding、navigation...等 | `TtlApplication,`<br>`TtlNavHost,`<br>`TopLevelDestination,`<br>`TtlApp,`<br>`TtlAppState` |    
| `feature:1`,<br>`feature:2`<br>... | 負責實現某個特定功能或用戶的互動流程的部分。這通常包含 UI 組件、UseCase 和 ViewModel，並從其他模組讀取資料。例如：<br>• [`feature:home`](https://github.com/azrael8576/TravelTaoyuanLite/tree/main/feature/home) 專注於展示首頁與個性化推薦。<br>• [`feature:attractions`](https://github.com/azrael8576/TravelTaoyuanLite/tree/main/feature/attractions) 提供桃園市多樣化景點資訊。 | `AttractionsScreen,`<br>`HomeScreen,`<br>... |    
| `core:data` | 負責從多個來源獲取應用程式的資料，並供其他功能模組共享。 | `TravelRepository,` <br>`SettingsRepository,` <br>`utils/ConnectivityManagerNetworkMonitor`|    
| `core:common` | 包含被多個模組共享的通用類別。<br>eg. 工具類、擴展方法...等 | `network/TtlDispatchers,`<br>`result/DataSourceResult,`<br>`manager/SnackbarManager,`<br>`extensions/StateFlowStateExtensions,`<br>`utils/UiText`<br>... |    
| `core:domain` | 包含被多個模組共享的 UseCase。 | None |    
| `core:model` | 提供整個應用程式所使用的模型類別。 | `TravelAttraction,`<br>`EventNews` |    
| `core:network` | 負責發送網絡請求，並處理來自遠程數據源的回應。 | `RetrofitTtlNetworkApi,`<br>`pagingsource/EventNewsPagingSource,`<br>`pagingsource/TravelAttractionsPagingSource,`<br>... |    
| `core:designsystem` | UI 依賴項。<br>eg. app theme、Core UI 元件樣式...等 | `TtlTheme,`<br>`TtlAppSnackbar`<br>... |    
| `core:testing` | 測試依賴項、repositories 和 util 類。 | `MainDispatcherRule,`<br>`TtlTestRunner,`<br>... |    
...
在 **旅桃Lite** 專案中，我們使用了 [_Hilt_](https://developer.android.com/training/dependency-injection/hilt-android?hl=en) 來進行依賴注入。而在資料層，我們將元件定義成接口形式，並依照具體需求進行實現綁定。

#### 策略亮點：
- **旅桃Lite** 並**未使用**任何 mocking libraries，而選擇使用 Hilt 的測試 API，方便我們將正式版本輕鬆替換成測試版本。
...
對於 UI Testing，**旅桃Lite** 採用了 [_Robot Testing Pattern_](https://jakewharton.com/testing-robots/?source=post_page-----fc820ce250f7--------------------------------)，其核心目的是建立一個抽象層，以聲明性的方式進行 UI 交互。
...
### 3. Screenshot tests
**旅桃Lite** 使用 [_Roborazzi_](https://github.com/takahirom/roborazzi) 進行特定畫面和組件的截圖測試。要運行這些測試，請執行 `verifyRoborazziDemoDebug` 或 `recordRoborazziDemoDebug` 任務。
...
**旅桃Lite** is distributed under the terms of the Apache License (Version 2.0). See the [license](https://github.com/azrael8576/TravelTaoyuanLite/blob/main/LICENSE) for more information.  
